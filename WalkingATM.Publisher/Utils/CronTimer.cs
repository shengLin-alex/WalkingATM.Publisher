        ...
        public ValueTask<bool> WaitForNextTickAsync(CronTimer owner, CancellationToken cancellationToken)
        {
            lock (this)
            {
                ...
                _ctr = cancellationToken.UnsafeRegister(
                    static (state, cancellationToken) => ((State)state!).Signal(cancellationToken: cancellationToken),
                    this);
            ...
            if (completeTask)
            {
                if (cancellationToken.IsCancellationRequested)
                {
                    ...
                    _mrvtsc.SetException(
                        ExceptionDispatchInfo.SetCurrentStackTrace(new OperationCanceledException(cancellationToken)));
                }
                else
                {
